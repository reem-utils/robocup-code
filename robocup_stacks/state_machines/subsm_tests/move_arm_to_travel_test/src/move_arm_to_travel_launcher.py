...
class DummyStateMachine(smach.State):
    ...
    def execute(self, userdata):
        print "Dummy state to go to  ArmMovementToTravelStateMachine initializing transformed_object_data and object_data"
        print "[            =D                ]"
...
def main():
    rospy.init_node('sm_ArmMovementToTravelStateMachine_test_state_machine')
    ...
    with sm:
        ...
        smach.StateMachine.add(
            'dummy_state',
            DummyStateMachine(),
            transitions={succeeded: 'ArmMovementToTravelStateMachine'})
    ...
    sis = smach_ros.IntrospectionServer(
        'sm_ArmMovementToTravelStateMachine_introspection', sm, '/SM_ROOT')
    sis.start()

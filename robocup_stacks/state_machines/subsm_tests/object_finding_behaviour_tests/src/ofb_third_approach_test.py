...
TEST_ROOM_NAME2 = 'living'
TARGET_OBJ = 'milk'
TSP_ROUTE = False


def main():
    rospy.init_node('object_finding_beh_third_approach_test')
    ...
    with sm:
        smach.StateMachine.add('START_CONTROLLERS', StartRobotControllers(head=True, left=True, right=True),
                               transitions={succeeded: 'OFB_THIRD_APPROACH_TEST2', aborted: aborted})

        '''smach.StateMachine.add('OFB_THIRD_APPROACH_TEST',
                               OFBThirdApproach(target_object_key='target_obj'),
        ...
        smach.StateMachine.add('PRINT_OBJECT_DATA', CBState(print_data, input_keys=['obj_data', 'room_name']),
                               transitions={succeeded: 'OFB_THIRD_APPROACH_TEST2'},
        ...
        smach.StateMachine.add('OFB_THIRD_APPROACH_TEST2',
                               OFBThirdApproach(tsp_route=TSP_ROUTE),
    ...
    sis = smach_ros.IntrospectionServer('ofb_third_approach_introspection', sm, '/SM_ROOT')
    sis.start()

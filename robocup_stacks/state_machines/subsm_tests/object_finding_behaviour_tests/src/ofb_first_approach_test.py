...
TEST_ROOM_NAME2 = 'kitchen'
TARGET_OBJ = 'juice'


def main():
    rospy.init_node('object_finding_beh_first_approach_test')
    ...
    with sm:
        smach.StateMachine.add('START_CONTROLLERS', StartRobotControllers(head=True, left=True, right=True),
                               transitions={succeeded: 'OFB_FIRST_APPROACH_TEST', aborted: aborted})

        smach.StateMachine.add('OFB_FIRST_APPROACH_TEST',
                               #OFBFirstApproach(TEST_ROOM_NAME),
                               OFBFirstApproach(target_object_key='target_obj'),
        ...
        smach.StateMachine.add('PRINT_OBJECT_DATA', CBState(print_data, input_keys=['obj_data', 'room_name']),
                               transitions={succeeded: 'OFB_FIRST_APPROACH_TEST2'},
        ...
        smach.StateMachine.add('OFB_FIRST_APPROACH_TEST2',
                               #OFBFirstApproach(TEST_ROOM_NAME2),
                               OFBFirstApproach(),
    ...
    sis = smach_ros.IntrospectionServer(
        'ofb_first_approach_introspection', sm, '/SM_ROOT')
    sis.start()

...
def cv_select_point(event, x, y, flags, param):
    ...
    if event and cv.CV_EVENT_RBUTTONDOWN:
        param.append((x,y))

def user_select_point(cvimage):
    """Select point based on user input."""
    cv.NamedWindow('select_point')
    cv.ShowImage('select_point', cvimage)
    selected_point=[]
    cv.SetMouseCallback('select_point', cv_select_point, selected_point)
...
def select_grasp_point_manual(req):
	...
	selected_point = user_select_point(image)
	maxp2du = selected_point[0][0]
	maxp2dv = selected_point[0][1]
	maxp2d = (maxp2du, maxp2dv)
